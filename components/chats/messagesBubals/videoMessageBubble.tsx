...
export default function VideoMessageBubble({ message }: VideoMessageBubbleProps) {
    ...
    return (
        <TouchableOpacity onPress={() => router.push({ pathname: "/chatWindow/mediaGallery" as any, params: { id: message?.file?.metadata?._id } })}>
            <Image
                source={message?.file?.metadata?.thumbnailUrl as ImageSourcePropType}
